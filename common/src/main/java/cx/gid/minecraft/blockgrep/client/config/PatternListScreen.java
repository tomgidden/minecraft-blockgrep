...
import cx.gid.minecraft.blockgrep.client.builder.EditablePattern;
import cx.gid.minecraft.blockgrep.client.builder.PatternEditorScreen;
...
public class PatternListScreen extends Screen
{
  ...
  /**
   * Width of the builder button when it sits inline, at the end of the
   * pattern field.
   */
  private static final int EDIT_BUTTON_WIDTH = 92;

  /**
   * Room the pattern field must keep for itself before the builder button is
   * allowed to share its row.
   *
   * Enough to show a short spec plus the start of the hint; below this the
   * field becomes a keyhole and the button is better off underneath, where it
   * can be full width and carry its longer label.
   */
  private static final int MIN_PATTERN_FIELD_WIDTH = 150;
  ...
  protected void init()
  {
    ...
    // [✓] Enable
    addRenderableWidget(
        Checkbox.builder(Component.translatable("blockgrep.screen.enable"), font)
    ...
    EditBox limitBox = new EditBox(
        font,
        limitX,
        HEADER_HEIGHT - 26,
        globalWidth,
        BUTTON_HEIGHT,
        Component.translatable("blockgrep.screen.limit")
    );
    ...
    editorPane = new ScrollPane(
        ...
        HEADER_HEIGHT,
        editorWidth(),
        height - HEADER_HEIGHT - BOTTOM_MARGIN
    );
    ...
    addRenderableWidget(
        Button
            .builder(
                Component.translatable("blockgrep.screen.add"),
                b -> {
            ...
            .bounds(GUTTER, listTop - BUTTON_HEIGHT - 4, LIST_WIDTH, BUTTON_HEIGHT)
            .build()
    );
    ...
    addRenderableWidget(
        Button
            .builder(
                Component.translatable("blockgrep.screen.done"),
                b -> onClose()
            )
            .bounds(width - GUTTER - 100, height - 28, 100, BUTTON_HEIGHT)
            .build()
    );
  ...
  private void select(SavedPattern pattern)
  {
    ...
    //
    // Pattern definition text field
    //
    y += LABEL_HEIGHT;

    // With room to spare the builder button tucks in at the end of the field
    // rather than taking a row of its own; the form is tall enough already.
    boolean inlineEdit = editButtonInline();
    int patternWidth   = inlineEdit
          ? paneWidth - EDIT_BUTTON_WIDTH - CELL_GAP
          : paneWidth;

    EditBox patternBox = new EditBox(
        font,
        left,
        y,
        patternWidth,
    ...
    //
    // Pattern builder button, beside the field or below it
    //
        ...
        Button
            .builder(
                Component.translatable(
                    inlineEdit
                        ? "blockgrep.editor.pattern.edit"
                        : "blockgrep.editor.pattern.editwith"
                ),
                b -> minecraft.setScreenAndShow(new PatternEditorScreen(this, pattern))
            )
            .bounds(
                inlineEdit ? left + patternWidth + CELL_GAP : left,
                inlineEdit ? y : y + BUTTON_HEIGHT + 4,
                inlineEdit ? EDIT_BUTTON_WIDTH : paneWidth,
                BUTTON_HEIGHT
            )
            .tooltip(net.minecraft.client.gui.components.Tooltip.create(
                Component.translatable(
                    PatternEditorScreen.isAvailable()
                        ? "blockgrep.editor.pattern.tooltip"
                        : "blockgrep.editor.pattern.unavailable"
                )
            ))
            .build();

    // The builder is built from item models, which do not exist until a
    // world has loaded. Opening it from the title screen used to crash the
    // client on the first frame.
    patternButton.active = PatternEditorScreen.isAvailable();

    addEditorWidget(patternButton);

    // Inline, the button shares the field's row and costs no extra height.
    // The summary line below is drawn by renderEditorLabels.
    y += BUTTON_HEIGHT;
    if(!inlineEdit) {
      y += 4 + BUTTON_HEIGHT;
    }
    y += 4 + LABEL_HEIGHT + FIELD_GAP;
    ...
    y += twoColumns ? 0 : CHECK_HEIGHT;
    addEditorWidget(mirrorBox("blockgrep.editor.mirror.vertical", rightColumn, y, true));
    ...
    addEditorWidget(new WidthSlider(left, y, twoColumns ? half : paneWidth, BUTTON_HEIGHT, pattern));
    y += BUTTON_HEIGHT + FIELD_GAP;
    ...
    int pickerWidth = Math.min(
        twoColumns ? half : paneWidth,
        ColorPicker.maxWidth()
    );
    colorsSideBySide = twoColumns;
    colorTop         = y;
    y += LABEL_HEIGHT + 2;

    strokePicker = new ColorPicker(font, left, y, pickerWidth, pattern.strokeColor, value -> {
    ...
    fillPicker = new ColorPicker(font, fillX, fillY, pickerWidth, pattern.fillColor, value -> {
  ...
  /**
   * Whether the builder button fits at the end of the pattern field.
   *
   * Consulted by both {@code select()}, which places the widgets, and
   * {@code renderEditorLabels()}, which walks the same vertical sequence to
   * position the labels.  Inline the button costs no height of its own, so
   * the two would drift apart if either decided this for itself.
   */
  private boolean editButtonInline()
  {
    return formWidth() - EDIT_BUTTON_WIDTH - CELL_GAP >= MIN_PATTERN_FIELD_WIDTH;
  }
  ...
  private Checkbox axisBox(String labelKey, int x, int y, int axis)
  {
    ...
    return Checkbox.builder(Component.translatable(labelKey), font)
        ...
        .onValueChange((box, value) -> {
          ...
          rot[axis]         = value;
          selected.symmetry = new Symmetry(rot[0], rot[1], rot[2], now.mirrorX(), now.mirrorY(), now.mirrorZ())
                                  .spec();
          edited();
        })
        .build();
  ...
  public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partial)
  {
    ...
    int limitX      = width - GUTTER - globalWidth;
    int radiusX     = limitX - GUTTER - globalWidth;
    graphics.text(font, Component.translatable("blockgrep.screen.limit"), limitX, HEADER_HEIGHT - 38, 0xFFA0A0A0);
    graphics.text(font, Component.translatable("blockgrep.screen.radius"), radiusX, HEADER_HEIGHT - 38, 0xFFA0A0A0);

    if(BlockGrepConfig.get().patterns.isEmpty()) {
      graphics.text(font, Component.translatable("blockgrep.screen.empty"), GUTTER + 4, HEADER_HEIGHT + LABEL_HEIGHT + BUTTON_HEIGHT + 14, 0xFFA0A0A0);
    }
  }
  ...
  private void renderEditorLabels(GuiGraphicsExtractor graphics)
  {
    ...
    graphics.text(font, Component.translatable("blockgrep.editor.name"), left, y, 0xFFA0A0A0);
    y += LABEL_HEIGHT + BUTTON_HEIGHT + FIELD_GAP;

    graphics.text(font, Component.translatable("blockgrep.editor.pattern"), left, y, 0xFFA0A0A0);
    y += LABEL_HEIGHT + BUTTON_HEIGHT;

    // Matches the step in select(): the builder button only adds a row when
    // it could not fit beside the field.
    if(!editButtonInline()) {
      y += 4 + BUTTON_HEIGHT;
    }
    y += 4;
    ...
    // boxes alone.
    graphics.text(font, Component.translatable("blockgrep.editor.orientations", describeSymmetry(selected.symmetry())), left, y, 0xFFA0A0A0);

    graphics.text(font, Component.translatable("blockgrep.editor.outline"), left, colorTop, 0xFFFFFFFF);
    graphics.text(font, Component.translatable("blockgrep.editor.fill"), colorsSideBySide ? left + half + 8 : left, fillTop, 0xFFFFFFFF);
  ...
  private static Component describeSymmetry(Symmetry symmetry)
  {
    ...
    return Component.translatable("blockgrep.symmetry.orientations", symmetry.transforms().size(), symmetry.spec());
  }
  ...
  private static PatternSummary summarize(String spec)
  {
    if(spec == null || spec.isBlank()) {
      return new PatternSummary(
          Component.translatable("blockgrep.editor.pattern.empty"),
          0xFFE05050
      );
    }
    try {
      ...
      return new PatternSummary(
          Component.translatable("blockgrep.editor.pattern.summary", pat.sizeX(), pat.sizeY(), pat.sizeZ(), pat.significantCells()),
          0xFF60D060
      );
    }
      ...
      try {
        ...
        return new PatternSummary(
            Component.translatable("blockgrep.editor.pattern.summary", parsed.sizeX(), parsed.sizeY(), parsed.sizeZ(), parsed.significantCells()),
      ...
      return new PatternSummary(
          Component.translatable("blockgrep.editor.pattern.invalid", e.getMessage() != null ? e.getMessage() : "syntax error"),
          0xFFE05050
      );
    }
  }
  ...
  private static class RadiusSlider extends AbstractSliderButton
  {
    ...
    {
      super(x, y, width, height, Component.empty(), (double) (config.radius - GrepState.MIN_RADIUS) / (GrepState.MAX_RADIUS - GrepState.MIN_RADIUS));
      this.config = config;
      updateMessage();
    }
  ...
  private Button linkButton(int x, int y, SavedPattern pattern)
  {
    Button button = Button.builder(linkLabel(pattern), b -> {
                            ...
                            edited();
                          }).bounds(x, y, LINK_WIDTH, LINK_HEIGHT).tooltip(net.minecraft.client.gui.components.Tooltip.create(Component.translatable("blockgrep.editor.link.tooltip"))).build();
    return button;
  }

  private static Component linkLabel(SavedPattern pattern)
  {
    return Component.translatable(pattern.linkColors ? "blockgrep.editor.link.on" : "blockgrep.editor.link.off");
  }
  ...
  private static class WidthSlider extends AbstractSliderButton
  {
    ...
    {
      super(x, y, width, height, Component.empty(), (pattern.strokeWidth - MIN) / (MAX - MIN));
    ...
    protected void updateMessage()
    {
      setMessage(Component.translatable("blockgrep.editor.width", String.format("%.1f", MIN + value * (MAX - MIN))));
    }
  ...
  private class PatternRow extends ObjectSelectionList.Entry<PatternRow>
  {
    ...
    public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float partial)
    {
      ...
      graphics.fill(rowLeft, rowY + 2, rowLeft + SWATCH_WIDTH, rowY + ROW_HEIGHT - 3, 0xFF000000 | (pattern.strokeColor & 0xFFFFFF));
      ...
      graphics.text(font, name, nameLeft, textY, pattern.enabled ? 0xFFFFFFFF : 0xFF808080);
