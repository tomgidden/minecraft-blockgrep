...
public class SavedPattern
{
   ...
   *
   * Also the alpha assumed when a config file gives a stroke color as six
   ...
   *
   * Also the alpha assumed when a config file gives a fill color as six hex
   * digits; see {@link HexColor}.
   */
  public static final int FILL_ALPHA = 0x26000000;

  /**
   * GSON requires a no-args constructor to deserialize entries.
   */
  public SavedPattern() {}
   ...
   */
  public SavedPattern(String name, String spec, Symmetry symmetry, int rgb, boolean enabled)
  {
    this.name        = name;
    this.spec        = spec;
  ...
  {
    try {
      return new ActivePattern(label(), compile(), symmetry(), strokeColor, fillColor, strokeWidth, xray);
    }
