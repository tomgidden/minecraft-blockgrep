...
public final class GrepCommand<S>
{
   ...
   */
  public static <S> void register(CommandDispatcher<S> dispatcher, CommandPlatform<S> platform)
  {
  ...
  private LiteralArgumentBuilder<S> onCommand()
  {
    return platform.literal("on")
        .executes(ctx -> {
          ...
          feedback(ctx.getSource(), Component.translatable("blockgrep.command.on"));
          return 1;
        });
  }
  ...
  private LiteralArgumentBuilder<S> listCommand()
  {
    return platform.literal("list")
        .executes(ctx -> {
          ...
          if(patterns.isEmpty()) {
            feedback(ctx.getSource(), Component.translatable("blockgrep.command.list.empty"));
            return 1;
          }
          feedback(ctx.getSource(), Component.translatable("blockgrep.command.list.header"));
          for(int i = 0; i < patterns.size(); i++) {
            SavedPattern pattern = patterns.get(i);
            feedback(ctx.getSource(), Component.translatable("blockgrep.command.list.entry", i + 1, Component.translatable(pattern.enabled ? "blockgrep.command.list.on" : "blockgrep.command.list.off"), pattern.label(), pattern.symmetry));
          }
          return 1;
        });
  }
    ...
    return platform.literal("toggle")
        .then(platform.argument("number", IntegerArgumentType.integer(1))
                  .executes(ctx -> {
                    ...
                    feedback(ctx.getSource(), Component.translatable(now ? "blockgrep.command.toggled.on" : "blockgrep.command.toggled.off", nameOf(oneBased - 1)));
                    return 1;
                  }));
  }
    ...
    return platform.literal("remove")
        .then(platform.argument("number", IntegerArgumentType.integer(1))
                  .executes(ctx -> {
                    ...
                    feedback(ctx.getSource(), Component.translatable("blockgrep.command.removed", name));
                    return 1;
                  }));
  }
        ...
        .then(platform.argument("number", IntegerArgumentType.integer(1))
                  .then(platform.argument("text", StringArgumentType.greedyString())
                            .executes(ctx -> {
                              ...
                              PatternManager.save();
                              feedback(ctx.getSource(), Component.translatable("blockgrep.command.renamed", nameOf(oneBased - 1)));
                              return 1;
                            })));
  }
    ...
    return platform.literal("only")
        .then(platform.argument("number", IntegerArgumentType.integer(1))
                  .executes(ctx -> {
                    ...
                    PatternManager.save();
                    feedback(ctx.getSource(), Component.translatable("blockgrep.command.only", nameOf(oneBased - 1)));
                    return 1;
                  }));
  }
  ...
  private int setAll(S source, boolean enabled)
  {
    ...
    PatternManager.save();
    feedback(source, Component.translatable(enabled ? "blockgrep.command.all.on" : "blockgrep.command.all.off", patterns.size()));
    return 1;
  }
  ...
  {
    error(source, Component.translatable("blockgrep.command.error.no_pattern", oneBased));
    return 0;
  }
                  ...
                  .then(platform.argument("sizeY", IntegerArgumentType.integer(1, 16))
                            .then(platform.argument("sizeZ", IntegerArgumentType.integer(1, 16))
                                      .then(platform.argument("blocks", StringArgumentType.greedyString())
                                                .executes(ctx -> applyBox(ctx.getSource(), IntegerArgumentType.getInteger(ctx, "sizeX"), IntegerArgumentType.getInteger(ctx, "sizeY"), IntegerArgumentType.getInteger(ctx, "sizeZ"), StringArgumentType.getString(ctx, "blocks")))))));
  }
  ...
  private LiteralArgumentBuilder<S> patternCommand()
  {
    return platform.literal("pattern")
        .then(platform.argument("spec", StringArgumentType.greedyString())
                  .executes(ctx -> applyPattern(ctx.getSource(), StringArgumentType.getString(ctx, "spec"))));
  }
  ...
  private LiteralArgumentBuilder<S> radiusCommand()
  {
    return platform.literal("radius")
        .then(platform.argument("blocks", IntegerArgumentType.integer(GrepState.MIN_RADIUS, GrepState.MAX_RADIUS))
                  .executes(ctx -> {
                    BlockGrepConfig.get().radius =
                        IntegerArgumentType.getInteger(ctx, "blocks");
                    BlockGrepConfig.get().apply();
                    BlockGrepConfig.save();
                    feedback(ctx.getSource(), Component.translatable("blockgrep.command.radius", GrepState.radius()));
                    return 1;
                  }));
  }
    ...
    return platform.literal("limit")
        .then(platform.argument("count", IntegerArgumentType.integer(1, 20000))
                  .executes(ctx -> {
                    ...
                    BlockGrepConfig.get().apply();
                    BlockGrepConfig.save();
                    feedback(ctx.getSource(), Component.translatable("blockgrep.command.limit", GrepState.limit()));
                    return 1;
                  }));
  }
  ...
  private LiteralArgumentBuilder<S> offCommand()
  {
    return platform.literal("off")
        .executes(ctx -> {
          ...
          feedback(ctx.getSource(), Component.translatable("blockgrep.command.off"));
          return 1;
        });
  }

  /**
   * {@code status} — summarize what is being searched for and found.
   */
    ...
    return platform.literal("status")
        .executes(ctx -> {
          if(!BlockGrepConfig.get().enabled) {
            feedback(ctx.getSource(), Component.translatable("blockgrep.command.status.disabled"));
            return 1;
          }
          ...
          if(live.isEmpty()) {
            feedback(ctx.getSource(), Component.translatable("blockgrep.command.status.none"));
            return PatternManager.errors().isEmpty() ? 1 : 0;
          }

          feedback(ctx.getSource(), Component.translatable("blockgrep.command.status.header", live.size(), GrepState.radius(), GrepState.currentHits().size()));
          for(ActivePattern entry: live) {
            long found = GrepState.currentHits().stream().filter(hit -> hit.source() == entry).count();
            feedback(ctx.getSource(), Component.translatable("blockgrep.command.status.entry", entry.name(), entry.pattern().description(), found));
          }
          return 1;
        });
  }

  private int applyBox(S source, int sx, int sy, int sz, String blocks)
  {
    try {
      ...
      SavedPattern added = PatternManager.addPattern(
          sx + "x" + sy + "x" + sz + " " + blocks.trim(),
          spec,
          Symmetry.YAW
      );
      feedback(source, Component.translatable("blockgrep.command.added", describeAdded(added, pattern)));
      return 1;
    }
  ...
  private int applyPattern(S source, String spec)
  {
    try {
      ...
      feedback(source, Component.translatable("blockgrep.command.added", describeAdded(added, parsed.pattern())));
      return 1;
    }
    catch(BlockPredicates.ParseException e) {
      error(source, e.getMessage());
  ...
  private static Component describeAdded(SavedPattern added, Pattern compiled)
  {
    ...
    return Component.translatable("blockgrep.command.added.detail", number, compiled.description(), describeSymmetry(added.symmetry()));
  }
