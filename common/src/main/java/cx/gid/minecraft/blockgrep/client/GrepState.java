...
public final class GrepState
{
   ...
   *
   * A single daemon thread rather than a pool: scans are serialized anyway by
   ...
   * chunk keeps the highlighted set roughly centered without rescanning on
   * every step.
   */
  ...
  public static void tick(Minecraft client)
  {
    ...
    synchronized(GrepState.class) {
      ...
      currentLimit     = limit;
      centerOfLastScan = lastScanCenter;
    ...
    boolean moved = centerOfLastScan == null
        || centerOfLastScan.distSqr(center) > (double) RESCAN_DISTANCE * RESCAN_DISTANCE;
