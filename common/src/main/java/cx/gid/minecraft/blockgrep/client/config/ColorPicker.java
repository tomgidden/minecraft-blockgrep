 ...
 * saturation track runs from gray to the current hue, and moving any slider
...
public class ColorPicker
{
   ...
   *
   * A color slider gains nothing from being 400px long — past a couple of
   ...
   * black or gray, and the player would be unable to drag it back out. Holding
  ...
  public ColorPicker(Font font, int x, int y, int available, int initial, IntConsumer onChange)
  {
    this.color    = initial;
    ...
    for(Channel channel: Channel.values()) {
      ...
      // the slider's label. In the label it was centered under the moving
      // handle, which made it hard to read while dragging and impossible
      // to type an exact value into.
      EditBox valueBox = new EditBox(font, sliderX + sliderWidth + CELL_GAP, row, VALUE_WIDTH, ROW_HEIGHT, Component.translatable(channel.labelKey));
    ...
    hexBox = new EditBox(font, sliderX, row, width - labelWidth - CELL_GAP - SWATCH_WIDTH - CELL_GAP, swatchHeight, Component.translatable("blockgrep.color.hex"));
  ...
  public void render(GuiGraphicsExtractor graphics, Font font)
  {
    for(ChannelSlider slider: sliders) {
      graphics.text(font, label(slider.channel), left, slider.getY() + 3, 0xFFA0A0A0);
    }
   ...
   */
  private static void drawChequer(GuiGraphicsExtractor graphics, int x, int y, int width, int height)
  {
    int cell = 4;
    for(int row = 0; row * cell < height; row++) {
      for(int col = 0; col * cell < width; col++) {
        int cx = x + col * cell;
        int cy = y + row * cell;
        graphics.fill(cx, cy, Math.min(cx + cell, x + width), Math.min(cy + cell, y + height), ((row + col) % 2 == 0) ? 0xFFB0B0B0 : 0xFF707070);
      }
    }
  }

  private static void drawOutline(GuiGraphicsExtractor graphics, int x, int y, int width, int height, int argb)
  {
  ...
  private class ChannelSlider extends AbstractSliderButton
  {
    ...
    public void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partial)
    {
      ...
      for(int i = 0; i < width; i++) {
        ...
        graphics.fill(x + i, y, x + i + 1, y + height, 0xFF000000 | (channel.sample(hsv, color, t) & 0xFFFFFF));
      }
      ...
      // Hollow, so the track color under the handle is the color the
      ...
      drawOutline(graphics, handleX + 1, y + 1, HANDLE_WIDTH - 2, height - 2, active ? 0xFFFFFFFF : 0xFFC6C6C6);
    }
    ...
    private int read()
    {
      ...
      return switch(channel) {
        ...
        // color that later went black or gray survives.
