 ...
 * north-south one.
 *
 * So {@code y} — equivalently {@code XyZ} — is the historical behavior of
...
public final class Symmetry
{
  ...
  public Symmetry(boolean freeX, boolean freeY, boolean freeZ, boolean mirrorX, boolean mirrorY, boolean mirrorZ)
  {
    this.freeX      = freeX;
    this.freeY      = freeY;
    this.freeZ      = freeZ;
  ...
  public static Symmetry parse(String spec) throws BlockPredicates.ParseException
  {
    ...
    return new Symmetry(rotations[0], rotations[1], rotations[2], mirrors[0], mirrors[1], mirrors[2]);
  }
   ...
   * recognizing an element it has already seen.
   */
  public record Transform(int mapX, int signX, int mapY, int signY, int mapZ, int signZ) {
    ...
    public Transform then(Transform next)
    {
      return new Transform(
          next.map(mapX),
          signX * next.sign(mapX),
          next.map(mapY),
          signY * next.sign(mapY),
          next.map(mapZ),
          signZ * next.sign(mapZ)
      );
    }
