...
public final class PatternManager
{
    ...
    for(SavedPattern saved: config.patterns) {
      ...
      try {
        ...
        failures.add(Component.translatable(
            "blockgrep.command.error.invalid",
            saved.label(),
            e.getMessage()
        ));
        ...
        Constants.LOGGER.warn("Pattern '{}' is invalid: {}", saved.label(), e.getMessage());
      }
    }
  ...
  {
    BlockGrepConfig config = BlockGrepConfig.get();
    SavedPattern added     = new SavedPattern(name, spec, symmetry, nextColor(config.patterns.size()), true);
